/*************************************************************************
	...
	> Created Time: Fri 06 Jan 2017 04:32:46 PM PST
 ************************************************************************/
...
namespace pr2_dynamic_movement_primitive_controller
{
    ...
    CartesianTwistControllerIkWithNullspaceOptimization::CartesianTwistControllerIkWithNullspaceOptimization() : robot_state_(NULL), jnt_to_twist_solver_(NULL), jnt_to_pose_solver_(NULL), jnt_to_jac_solver_(NULL), num_joints_(0), publisher_counter_(0), publisher_buffer_size_(0), header_sequence_number_(0)
    {
    }
    ...
    bool CartesianTwistControllerIkWithNullspaceOptimization::initRTPublisher()
    {
        ...
        pose_twist_desired_publisher_.reset(new rosrt::Publisher<pr2_dynamic_movement_primitive_controller::PoseTwistStamped>(node_handle_.advertise<pr2_dynamic_movement_primitive_controller::PoseTwistStamped> (std::string("pose_twist_desired"), 1), publisher_buffer_size_, pose_twist_stamped_desired_msg));

        pr2_dynamic_movement_primitive_controller::PoseTwistStamped pose_twist_stamped_actual_msg;
        pose_twist_actual_publisher_.reset(new rosrt::Publisher<pr2_dynamic_movement_primitive_controller::PoseTwistStamped>(node_handle_.advertise<pr2_dynamic_movement_primitive_controller::PoseTwistStamped> (std::string("pose_twist_actual"), 1), publisher_buffer_size_, pose_twist_stamped_actual_msg));

        pr2_dynamic_movement_primitive_controller::NullspaceTermStamped nullspace_term_msg;
        nullspace_term_publisher_.reset(new rosrt::Publisher<pr2_dynamic_movement_primitive_controller::NullspaceTermStamped>(node_handle_.advertise<pr2_dynamic_movement_primitive_controller::NullspaceTermStamped> (std::string("nullspace_term"), 1), publisher_buffer_size_, nullspace_term_msg));
    ...
    void CartesianTwistControllerIkWithNullspaceOptimization::update()
    {
        ...
        eigen_jac_times_jac_transpose_ = eigen_jjt_inverse_.inverse();
        ...
        for (int i = 0; i < 3; ++i)
        {
            eigen_desired_cartesian_velocities_(i) = (kdl_twist_desired_(i) * ff_trans_) + cartesian_fb_pid_controllers_[i].computeCommand(kdl_twist_error_(i), dt_);
        }
        for (int i = 3; i < NUM_CART; ++i)
        {
            eigen_desired_cartesian_velocities_(i) = (kdl_twist_desired_(i) * ff_rot_) + cartesian_fb_pid_controllers_[i].computeCommand(kdl_twist_error_(i), dt_);
        }
        ...
        for (int i = 0; i < num_joints_; ++i)
        {
            ...
            eigen_nullspace_error_(i) = nullspace_fb_pid_controllers_[i].computeCommand(error, dt_);
        }
