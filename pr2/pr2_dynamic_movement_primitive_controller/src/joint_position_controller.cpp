/*************************************************************************
	...
	> Created Time: Fri 06 Jan 2017 03:55:36 PM PST
 ************************************************************************/
...
{
    JointPositionController::JointPositionController() : initialized_(false), joint_state_(NULL), command_(0), commanded_effort_(0), publisher_rate_(1), publisher_counter_(0), publisher_buffer_size_(1000), error_(0.0), error_dot_(0.0), last_error_(0.0), robot_(NULL), last_time_(0)
    {
    }
                                       ...
                                       ros::NodeHandle &node_handle)
    {
        ...
        controller_state_publisher_.reset(new rosrt::Publisher<pr2_controllers_msgs::JointControllerState>(node_handle_.advertise<pr2_controllers_msgs::JointControllerState>(std::string("state"), 1), publisher_buffer_size_, joint_controller_state_msg));
    ...
    void JointPositionController::update()
    {
        ...
        {
            angles::shortest_angular_distance_with_limits(command_, joint_state_->position_, joint_state_->joint_->limits->lower, joint_state_->joint_->limits->upper, error_);
        }
        ...
        commanded_effort_ = pid_controller_.computeCommand(error_, error_dot_, dt_);
