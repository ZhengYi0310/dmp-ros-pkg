/*************************************************************************
	> File Name: skill_library_client.cpp
	> Author: Yi Zheng 
	> Mail: hczhengcq@gmail.com
	> Created Time: Mon 26 Dec 2016 03:10:37 PM PST
 ************************************************************************/
...
{
    bool SkillLibraryClient::get(const std::string& dmp_name, dmp_lib::DMPBasePtr& dmp)
    {
        if (dmp_name.empty())
        {
        ...
        {
            ROS_ERROR("Retrieving DMP with name >%s< from the skill library failed.", dmp_name.c_str());
            return false;
        }

        affordance = get_affordance_service.response.affordance;
        affordance.dmp.dmp_version = dynamic_movement_primitive::DMPUtilitiesMsg::NC2010;
