/*************************************************************************
	> File Name: dynamic_movement_primitive_base.cpp
	> Author: Yi Zheng 
	> Mail: hczhengcq@gmail.com
	> Created Time: Mon 31 Oct 2016 12:20:36 PM PDT
 ************************************************************************/
...
namespace dmp_lib
{
    ...
    bool DynamicMovementPrimitiveBase::initialize(DMPParamBasePtr parameters,
                                                  ...
                                                  CSBasePtr canonical_system)
    {
        ...
        //Logger::logPrintf("transformation_systems size: %i", Logger::DEBUG, (int)transformation_systems.size());
        if(!setupIndices())
        {
    ...
    bool DynamicMovementPrimitiveBase::initialize(DMPParamBasePtr parameters,
                                                  vector<TSBasePtr>& transformation_systems,
                                                  CSBasePtr canonical_system)
    {
        DMPStateBasePtr state(new DynamicMovementPrimitiveStateBase());
        return initialize(parameters, state, transformation_systems, canonical_system);
    }

    bool DynamicMovementPrimitiveBase::setupIndices()
    {
        indices_.clear();
    ...
    bool DynamicMovementPrimitiveBase::isCompatible(const DynamicMovementPrimitiveBase& other_dmp) const
    {
        if (!initialized_)
        {
    ...
    void DynamicMovementPrimitiveBase::changeType(const int type)
    {
        assert(initialized_);
        parameters_->type_ = type;
    }

    bool DynamicMovementPrimitiveBase::hasType(const int type) const
    {
        assert(initialized_);
        return (type == parameters_->type_);
    }

    int DynamicMovementPrimitiveBase::getType() const
    {
        assert(initialized_);
        return parameters_->type_;
    }

    int DynamicMovementPrimitiveBase::getSeq() const
    {
        assert(initialized_);
        return state_->seq_;
    }

    void DynamicMovementPrimitiveBase::setId(const int id)
    {
        assert(initialized_);
        parameters_->id_ = id;
    }

    int DynamicMovementPrimitiveBase::getId() const
    {
        assert(initialized_);
        return parameters_->id_;
    }

    bool DynamicMovementPrimitiveBase::get(DMPParamBaseConstPtr& parameters,
                                           DMPStateBaseConstPtr& state) const
    {
        if (!initialized_)
        {
    ...
    bool DynamicMovementPrimitiveBase::prepareTrajectory(Trajectory& trajectory)
    {
        ...
        {
            Logger::logPrintf("Trajectory has >%i< samples, but must have at least >%i<. Cannot prepare trajectory.", Logger::ERROR, trajectory.getNumContainedSamples(), MIN_NUM_DATA_POINTS);
            return false;
        }
    ...
    bool DynamicMovementPrimitiveBase::learnFromThetas(const std::vector<Eigen::VectorXd>& thetas,
                                                       ...
                                                       const double initial_duration)
    {
        ...
        {
            Logger::logPrintf("There are >%i< parameter vectors, but the DMP has >%i< dimensions.", Logger::ERROR, thetas.size(), getNumDimensions());
            return (state_->is_learned_ = false);
        }
        if(initial_start.size() != getNumDimensions() || initial_goal.size() != getNumDimensions())
        {
            Logger::logPrintf("Size of provided initial start >%i< or initial goal >%i< does not match number of dimensions of the DMP.", Logger::ERROR, initial_start.size(), initial_goal.size(), getNumDimensions());
            return (state_->is_learned_ = false);
        }
    ...
    bool DynamicMovementPrimitiveBase::learnFromTrajectory(const Trajectory& demo_trajectory, TrajectoryPtr debug_trajectory)
    {
        assert(initialized_);
        assert(demo_trajectory.isInitialized());
        Logger::logPrintf("Learning >%i< dimensional >%s< DMP from trajectory with >%i< dimensions.", Logger::INFO, getNumDimensions(), getVersionString().c_str(), demo_trajectory.getDimension());
    ...
    bool DynamicMovementPrimitiveBase::learnFromMinimumJerk(const Eigen::VectorXd& start,
    ...
    bool DynamicMovementPrimitiveBase::learnFromMinimumJerk(const std::vector<double>& start,
    ...
    bool DynamicMovementPrimitiveBase::learnFromMinimumJerk(const std::vector<Eigen::VectorXd>& waypoints,
                                                        const double sampling_frequency,
                                                        ...
                                                        TrajectoryPtr debug_trajectory)
    {
        ...
        {
            Logger::logPrintf("There are >%i< initial durations for >%i< waypoints. Cannot learn DMP minimum jerk trajectory.", Logger::ERROR, initial_durations.size()+1, waypoints.size());
            return false;
        }
        ...
        Logger::logPrintf("Learning DMP from minimum jerk trajectory with >%i< waypoints, >%i< samples, and sampled at >%.1f< Hz.", Logger::DEBUG, (int)waypoints.size(), num_total_samples, sampling_frequency);
    ...
    bool DynamicMovementPrimitiveBase::learnFromMinimumJerk(const std::vector<std::vector<double> >& waypoints,
                                                            const double sampling_frequency,
                                                            const std::vector<double>& initial_durations,
                                                            TrajectoryPtr debug_trajectory)
    {
    ...
    bool DynamicMovementPrimitiveBase::createDebugTrajectory(Trajectory& debug_trajectory, const Trajectory& trajectory)
    {
    ...
    bool DynamicMovementPrimitiveBase::logDebugTrajectory(Trajectory& debug_trajectory)
    {
    ...
    bool DynamicMovementPrimitiveBase::learnTransformationTarget()
    {
        assert(initialized_);
        for (int i = 0; i < getNumDimensions(); ++i)
        {
            // ignore the first dimension of the quaternion transformation system
            if (!(transformation_systems_[indices_[i].first]->integration_method_ == TransformationSystemBase::QUATERNION && indices_[i].second == 0))
            {
                ...
                if (transformation_systems_[indices_[i].first]->states_[indices_[i].second]->function_input_.size() != transformation_systems_[indices_[i].first]->states_[indices_[i].second]->function_target_.size())
                {
                    Logger::logPrintf("Transformaion system >%i< dimension >%i< has incompatible function sizes (input >%i< vs. target = >%i<).", Logger::ERROR, indices_[i].first, indices_[i].second, (int)transformation_systems_[indices_[i].first]->states_[indices_[i].second]->function_input_.size(), (int)transformation_systems_[indices_[i].first]->states_[indices_[i].second]->function_target_.size());
                    return false;
                }

                Eigen::Map<VectorXd> input = VectorXd::Map(&transformation_systems_[indices_[i].first]->states_[indices_[i].second]->function_input_[0], transformation_systems_[indices_[i].first]->states_[indices_[i].second]->function_input_.size());
                Eigen::Map<VectorXd> target = VectorXd::Map(&transformation_systems_[indices_[i].first]->states_[indices_[i].second]->function_target_[0], transformation_systems_[indices_[i].first]->states_[indices_[i].second]->function_target_.size());
    ...
    bool DynamicMovementPrimitiveBase::setup(const VectorXd& start,
                                             const VectorXd& goal,
    ...
    bool DynamicMovementPrimitiveBase::setupDuration(const VectorXd& start,
                                                     const VectorXd& goal,
                                                     const double movement_duration)
    {
        assert(initialized_);
    ...
    bool DynamicMovementPrimitiveBase::setupSamplingFrequency(const VectorXd& start,
                                                              const VectorXd& goal,
                                                              const double sampling_frequency)
    {
        assert(initialized_);
    ...
    bool DynamicMovementPrimitiveBase::setup(const VectorXd &goal,
                                             const double movement_duration,
                                             const double sampling_frequency)
    {
        assert(initialized_);
    ...
    bool DynamicMovementPrimitiveBase::setupDuration(const Eigen::VectorXd& goal,
                                                     const double movement_duration)
    {
        assert(initialized_);
    ...
    bool DynamicMovementPrimitiveBase::setupSamplingFrequency(const Eigen::VectorXd& goal,
                                                              const double sampling_frequency)
    {
        assert(initialized_);
    ...
    bool DynamicMovementPrimitiveBase::setupSamplingFrequency(const double sampling_frequency)
    {
        assert(initialized_);
        VectorXd goal = VectorXd::Zero(getNumDimensions());
        for (int i = 0; i < getNumDimensions(); i++)
    ...
    bool DynamicMovementPrimitiveBase::setupDuration(const double movement_duration)
    {
        assert(initialized_);
        VectorXd goal = VectorXd::Zero(getNumDimensions());
        for (int i = 0; i < getNumDimensions(); i++)
    ...
    bool DynamicMovementPrimitiveBase::setup()
    {
        assert(initialized_);
        VectorXd goal = VectorXd::Zero(getNumDimensions());
        for (int i = 0; i < getNumDimensions(); i++)
    ...
    vector<string> DynamicMovementPrimitiveBase::getVariableNames() const
    {
        assert(initialized_);
        vector < string > variable_names;
        for (vector<TSBasePtr>::const_iterator ti = transformation_systems_.begin(); ti != transformation_systems_.end(); ++ti)
        {
    ...
    bool DynamicMovementPrimitiveBase::getGoal(const std::vector<std::string> variable_names, std::vector<double> &goal) const
    {
        assert(initialized_);
        goal.clear();
    ...
    bool DynamicMovementPrimitiveBase::getStart(const std::vector<std::string> variable_names, std::vector<double> &start) const
    {
        assert(initialized_);
        start.clear();
    ...
    bool DynamicMovementPrimitiveBase::isReadyToPropagate()
    {
        if (!state_->is_learned_)
        {
    ...
    bool DynamicMovementPrimitiveBase::propagateFull(Trajectory& trajectory,
    ...
    bool DynamicMovementPrimitiveBase::integrate(const int num_iteration, const VectorXd& feedback)
    {
        assert(initialized_);
        int index = 0;
        for (int i = 0; i < getNumTransformationSystems(); ++i)
    ...
    // REAL-TIME REQUIREMENTS
    bool DynamicMovementPrimitiveBase::propagateStep(VectorXd& desired_positions,
                                                     ...
                                                     const int num_samples)
    {
        ...
        if ((desired_positions.size() != desired_velocities.size()) || (desired_positions.size() != desired_accelerations.size()) || (desired_positions.size() < getNumDimensions()))
        {
            Logger::logPrintf("Number of desired positions >%i<, velocities >%i<, or accelerations >%i< is incorrect, it should be >%i<. (Real-time violation).", Logger::ERROR, desired_positions.size(), desired_velocities.size(), desired_accelerations.size(), getNumDimensions());
        ...
        {
            Logger::logPrintf("Size of feedback vector >%i< does not match number of dimension >%i<. (Real-time violation).", Logger::ERROR, feedback.size(), getNumDimensions());
            movement_finished = true;
            return false;
        }
    ...
    bool DynamicMovementPrimitiveBase::getNumRFS(vector<int>& num_rfs) const
    {
        assert(initialized_);
        num_rfs.clear();
        for (int i = 0; i < getNumDimensions(); ++i)
    ...
    bool DynamicMovementPrimitiveBase::getThetas(vector<VectorXd>& thetas) const
    {
        assert(initialized_);
        thetas.clear();
        for (int i = 0; i < getNumDimensions(); ++i)
    ...
    bool DynamicMovementPrimitiveBase::setThetas(const vector<VectorXd>& thetas)
    {
        assert(initialized_);
        if(static_cast<int> (thetas.size()) != getNumDimensions())
        {
            Logger::logPrintf("Size of theta vector >%i< does not match number of DMP dimensions. Cannot set thetas.", Logger::ERROR, (int)thetas.size(), getNumDimensions());
            return false;
        }
        for (int i = 0; i < getNumDimensions(); ++i)
        {
    ...
    bool DynamicMovementPrimitiveBase::generateBasisFunctionMatrix(const int num_time_steps, vector<MatrixXd>& basis_functions) const
    {
        assert(initialized_);
        if(num_time_steps <= 0)
        {
    ...
    bool DynamicMovementPrimitiveBase::getBasisFunctionCenters(vector<VectorXd>& basis_function_centers) const
    {
        assert(initialized_);
    ...
    bool DynamicMovementPrimitiveBase::getCanonicalSystemState(double& x, double& xd) const
    {
        assert(initialized_);
