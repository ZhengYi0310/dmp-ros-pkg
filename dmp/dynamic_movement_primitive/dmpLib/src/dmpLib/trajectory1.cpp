/*************************************************************************
	...
	> Created Time: Fri 28 Oct 2016 11:22:02 AM PDT
 ************************************************************************/
...
// 32 bit word byte/word swap marcos
...
namespace dmp_lib
{
    ...
    {
        Logger::logPrintf(positions_only, "initialize position trajecotry with >%i< dimensions, with a sampling frequency of >%.1f< Hz and maximal length of >%i< samples.",
                          Logger::DEBUG, (int)variable_names.size(), sampling_frequency, trajectory_length);
        Logger::logPrintf(!positions_only, "initialize position, velocity and acceleration trajecotry with >%i< dimensions, with a sampling frequency of >%.1f< Hz and maximal length of >%i< samples.",
                          Logger::DEBUG, (int)variable_names.size(), sampling_frequency, trajectory_length);
    ...
    bool Trajectory::initializeWithMinJerk(const std::vector<std::string> & variable_names,
                                           ...
                                           const int trajectory_length)
    {
        return initializeWithMinJerk(variable_names, sampling_frequency, duration, VectorXd::Map(&start[0], start.size()), VectorXd::Map(&goal[0], goal.size()), positions_only, trajectory_length);
    }
    ...
    bool Trajectory::setMinJerk(const Eigen::VectorXd& start, const Eigen::VectorXd& goal, const int num_samples)
    {
        ...
        {
            Logger::logPrintf("Trajectory dimension >%i< does not match size of provided start >%i< or goal >%i<.", Logger::ERROR, trajectory_dimension_, start.size(), goal.size());
            return false;
        }

        if (trajectory_length_ < num_samples)
        {
            Logger::logPrintf("Cannot set minimum jerk trajectory with >%i< samples because trajectory is only of length >%i<.", Logger::ERROR, num_samples, trajectory_length_);
            return false;
        }
        ...
        Logger::logPrintf("Setting minimum jerk trajectory with >%i< samples using a sampling frequency of >%.1f< Hz and a duration of >%.1f< seconds.", Logger::DEBUG, num_samples, sampling_frequency_, duration);
        ...
        for (int i = 1; i < num_samples; i++)
        {
            for (int j = 0; j < trajectory_dimension_; j++)
            {
                ...
                {
                    Logger::logPrintf("Could not compute next minimum jerk trajecotry point. Cannot set minimum jerk trajecotry.", Logger::ERROR);
                    return false;
                }
            ...
            if (!add(next_position, next_velocity, next_acceleration, positions_only_))
            {
                Logger::logPrintf("Could not add first trajectory point. Cannot set minimum jerk trajecotry.", Logger::ERROR);
                return false;
            }
        }
    ...
    bool Trajectory::setMinJerk(const std::vector<Eigen::VectorXd>& waypoints, const std::vector<int>& num_samples)
    {
        ...
        {
            Logger::logPrintf("Number of waypoints >%i< does not match number of num_samples provided >%i<. Cannot set minimum jerk trajectory.", Logger::ERROR, (int)waypoints.size(), (int)num_samples.size()+1);
            return false;
        }

        if(waypoints.size() < 2)
        {
            Logger::logPrintf("Number of waypoints >%i< provided must be greater or equal 2. Cannot set minimum jerk trajectory.", Logger::ERROR, (int)waypoints.size());
            return false;
        }
    ...
    bool Trajectory::readFromCLMCFile(const string& file_name, const bool positions_only)
    {
                                      ...
                                      const bool use_variable_names)
    {
        ...
        {
            Logger::logPrintf("Values for number of columns >%i< and rows >%i< are out of bound (%i  x %i).", Logger::ERROR, tmp_num_cols, tmp_num_rows, ABSOLUTE_MAX_TRAJECTORY_DIMENSION, ABSOLUTE_MAX_TRAJECTORY_LENGTH);
            fclose(fp);
            return false;
        }
        ...
        Logger::logPrintf(positions_only, "Read position trajectory containing >%i< variables with each >%i< data points.", Logger::INFO, (int)variable_names_list.size(), trajectory_length_);
        Logger::logPrintf(!positions_only, "Read trajectory containing position, velocity, and acceleration of >%i< variables with each >%i< data points.", Logger::INFO, (int)variable_names_list.size(), trajectory_length_);
                                     ...
                                     const bool positions_only) const
    {
        ...
        Logger::logPrintf(positions_only, "Writing trajectory with >%i< position traces and >%i< samples to clmc file >%s<.", Logger::INFO, trajectory_dimension_, index_to_last_trajectory_point_, file_name.c_str());
        Logger::logPrintf(!positions_only, "Writing trajectory with >%i< position, velocity and acceleration traces and >%i< samples to clmc file >%s<.", Logger::INFO, trajectory_dimension_, index_to_last_trajectory_point_, file_name.c_str());
    ...
    bool Trajectory::rearange(const vector<string>& variable_names_order)
    {
        ...
        {
            Logger::logPrintf("Number of variable names >%i< does not match number of variables >%i< contained in trajectory. Cannot rearange trajectory.", Logger::ERROR, (int)variable_names_order.size(), (int)variable_names_.size());
            return false;
        }
    ...
    bool Trajectory::onlyKeep(const vector<string>& variable_names)
    {
        ...
        {
            Logger::logPrintf("Number of variable names >%i< must be less than number of variables >%i< contained in trajectory. Cannot reduce trajectory dimension.", Logger::ERROR, (int)variable_names.size(), (int)variable_names_.size());
            return false;
        }
    ...
    bool Trajectory::getTrajectoryPoint(const int trajectory_index, VectorXd& trajectory_point) const
    {
    ...
    bool Trajectory::setTrajectoryPoint(const int trajectory_index, const VectorXd& trajectory_point)
    {
        if(!isWithinLengthBoundaries(trajectory_index))
        {
            Logger::logPrintf("Trajectory index >%i< is out of bound [0..%i].", Logger::ERROR, trajectory_index, trajectory_length_);
    ...
    bool Trajectory::computeDerivatives(const int num_extra_points)
    {
        ...
        for (int i = 0; i < vel_length; i++)
        {
            for (int j = 0; j < trajectory_dimension_; j++)
            {
                tmp_trajectory_vel(i, j) = (tmp_trajectory_pos(i, j) - (static_cast<double> (8.0) * tmp_trajectory_pos(i + 1, j)) + (static_cast<double> (8.0) * tmp_trajectory_pos(i + 3, j)) - tmp_trajectory_pos(i + 4, j)) / static_cast<double> (12.0);
        ...
        for (int i = 0; i < acc_length; i++)
        {
            for (int j = 0; j < trajectory_dimension_; j++)
            {
                tmp_trajectory_acc(i, j) = (tmp_trajectory_vel(i, j) - (static_cast<double> (8.0) * tmp_trajectory_vel(i + 1, j)) + (static_cast<double> (8.0) * tmp_trajectory_vel(i + 3, j)) - tmp_trajectory_vel(i + 4, j)) / static_cast<double> (12.0);
    ...
    bool Trajectory::crop(const int num_trajectory_points, bool crop_the_end)
    {
        ...
        if(crop_the_end)
        {
            ...
            trajectory_data.block(0, 0, new_index_to_last_trajectory_point, trajectory_dimension_) = trajectory_positions_.block(0, 0, new_index_to_last_trajectory_point, trajectory_dimension_);
            trajectory_positions_ = trajectory_data;
            if(!positions_only_)
            {
                trajectory_data.setZero(trajectory_length_, trajectory_dimension_);
                trajectory_data.block(0, 0, new_index_to_last_trajectory_point, trajectory_dimension_) = trajectory_velocities_.block(0, 0, new_index_to_last_trajectory_point, trajectory_dimension_);
                trajectory_velocities_ = trajectory_data;
                trajectory_data.setZero(trajectory_length_, trajectory_dimension_);
                trajectory_data.block(0, 0, new_index_to_last_trajectory_point, trajectory_dimension_) = trajectory_accelerations_.block(0, 0, new_index_to_last_trajectory_point, trajectory_dimension_);
        ...
        {
            trajectory_data.setZero(trajectory_length_, trajectory_dimension_);
            trajectory_data.block(0, 0, new_index_to_last_trajectory_point, trajectory_dimension_) = trajectory_positions_.block(num_trajectory_points, 0, new_index_to_last_trajectory_point, trajectory_dimension_);
            trajectory_positions_ = trajectory_data;
            if(!positions_only_)
            {
                trajectory_data.setZero(trajectory_length_, trajectory_dimension_);
                trajectory_data.block(0, 0, new_index_to_last_trajectory_point, trajectory_dimension_) = trajectory_velocities_.block(num_trajectory_points, 0, new_index_to_last_trajectory_point, trajectory_dimension_);
                trajectory_velocities_ = trajectory_data;

                trajectory_data.setZero(trajectory_length_, trajectory_dimension_);
                trajectory_data.block(0, 0, new_index_to_last_trajectory_point, trajectory_dimension_) = trajectory_accelerations_.block(num_trajectory_points, 0, new_index_to_last_trajectory_point, trajectory_dimension_);
                trajectory_accelerations_ = trajectory_data;
            }
        }
                         ...
                         const bool positions_only)
    {
        ...
        if (sampling_frequency_ <= 0.0)
        {
            Logger::logPrintf("Sampling frequency >%.1f< is invalid. Not adding trajectory point (Real-time violation). Cannot add sample to trajectory.", Logger::ERROR, sampling_frequency_);
            return false;
        }
                         ...
                         const bool positions_only)
    {
        ...
        {
            Logger::logPrintf("Size of provided vector >%i< does not match trajectory dimension >%i< (Real-time violation). Cannot add sample to trajectory.", Logger::FATAL, (int)trajectory_positons.size(), trajectory_dimension_);
            return false;
        }
        if (sampling_frequency_ <= 0.0)
        {
                                         ...
                                         VectorXd& nmse_of_positions) const
    {
        ...
        {
            Logger::logPrintf("Provided vector to hold normalized MSE has size >%i<, but it should have size >%i<.", Logger::ERROR, nmse_of_positions.size(), trajectory_dimension_);
            return false;
        }
        ...
        Logger::logPrintf("Computing normalized mean squared error between two trajectories with >%i< dimensions and >%i< samples.", Logger::DEBUG, trajectory_dimension_, index_to_last_trajectory_point_);
        ...
        nmse_of_positions = (trajectory_positions_.block(0, 0, index_to_last_trajectory_point_, trajectory_dimension_) - trajectory.trajectory_positions_.block(0, 0, index_to_last_trajectory_point_, trajectory_dimension_)).array().square().matrix().colwise().sum();
    ...
    bool Trajectory::cut(Trajectory& other_trajectory, bool verbose)
    {
        ...
        {
            Logger::logPrintf(verbose, "Trajectories do not have same sampling frequency (>%.3f< Hz vs. >%.3f< Hz). Cannot cut them.", Logger::ERROR, sampling_frequency_, other_trajectory.sampling_frequency_);
            return false;
        }

        if (index_to_last_trajectory_point_ < other_trajectory.index_to_last_trajectory_point_)
        {
            Logger::logPrintf(verbose, "Cutting trajectory to contain >%i< samples with duration >%.1f< seconds and sampled at >%.1f< Hz.", Logger::INFO, index_to_last_trajectory_point_, sampling_frequency_, trajectory_duration_);
        ...
        {
            Logger::logPrintf(verbose, "Cutting trajectory to contain >%i< samples with duration >%.1f< seconds and sampled at >%.1f< Hz.", Logger::INFO, other_trajectory.index_to_last_trajectory_point_, other_trajectory.sampling_frequency_, other_trajectory.trajectory_duration_);
    ...
    bool Trajectory::isCompatible(const Trajectory& other_trajectory, bool verbose) const
    {
        ...
        if (index_to_last_trajectory_point_ != other_trajectory.index_to_last_trajectory_point_)
        {
            Logger::logPrintf(verbose, "Trajectories do not have same index to last trajectory point (>%i< vs. >%i<).", Logger::ERROR, index_to_last_trajectory_point_, other_trajectory.index_to_last_trajectory_point_);
            return false;
        }
  
        if (fabs(sampling_frequency_ - other_trajectory.sampling_frequency_) > 1e-3)
        {
            Logger::logPrintf(verbose, "Trajectories do not have same sampling frequency (>%.3f< Hz vs. >%.3f< Hz).", Logger::ERROR, sampling_frequency_, other_trajectory.sampling_frequency_);
            return false;
        }

        if (fabs(trajectory_duration_ - other_trajectory.trajectory_duration_) > 1e-3)
        {
            Logger::logPrintf(verbose, "Trajectories do not have same duration (>%.3f< seconds vs. >%.3f< seconds).", Logger::ERROR, trajectory_duration_, other_trajectory.trajectory_duration_);
            return false;
        }
    ...
    bool Trajectory::isAppendable(const Trajectory& other_trajectory, bool verbose) const
    {
        ...
        {
            Logger::logPrintf(verbose, "Trajectories do not have same dimensions (>%i< vs. >%i<).", Logger::ERROR, trajectory_dimension_, other_trajectory.trajectory_dimension_);
            return false;
        }
    ...
    bool Trajectory::combine(const Trajectory& other_trajectory, bool verbose)
    {
        ...
        new_trajectory_positions.block(0,0,index_to_last_trajectory_point_,trajectory_dimension_) = trajectory_positions_.block(0,0,index_to_last_trajectory_point_,trajectory_dimension_);

        if(!positions_only_)
        {
            MatrixXd new_trajectory_velocities = MatrixXd::Zero(trajectory_length_, new_trajectory_dimension);
            new_trajectory_velocities.block(0,0,index_to_last_trajectory_point_,trajectory_dimension_) = trajectory_velocities_.block(0,0,index_to_last_trajectory_point_,trajectory_dimension_);
            new_trajectory_velocities.block(0,trajectory_dimension_,index_to_last_trajectory_point_,other_trajectory.trajectory_dimension_) = other_trajectory.trajectory_velocities_.block(0,0,index_to_last_trajectory_point_,other_trajectory.trajectory_dimension_);
            ...
            new_trajectory_accelerations.block(0,0,index_to_last_trajectory_point_,trajectory_dimension_)= trajectory_accelerations_.block(0,0,index_to_last_trajectory_point_,trajectory_dimension_);
            new_trajectory_accelerations.block(0,trajectory_dimension_,index_to_last_trajectory_point_,other_trajectory.trajectory_dimension_) = other_trajectory.trajectory_accelerations_.block(0,0,index_to_last_trajectory_point_,other_trajectory.trajectory_dimension_);
    ...
    bool Trajectory::append(const Trajectory& other_trajectory)
    {
        ...
        if (index_to_last_trajectory_point_ + trajectory.index_to_last_trajectory_point_ > trajectory_length_)
        {
            ...
            new_trajectory_positions.block(0,0,index_to_last_trajectory_point_,trajectory_dimension_) = trajectory_positions_.block(0,0,index_to_last_trajectory_point_,trajectory_dimension_);
            new_trajectory_positions.block(index_to_last_trajectory_point_,0,trajectory.index_to_last_trajectory_point_,trajectory_dimension_) = trajectory.trajectory_positions_.block(0,0,trajectory.index_to_last_trajectory_point_,trajectory_dimension_);
            ...
            if(!positions_only_)
            {
                ...
                new_trajectory_velocities.block(0,0,index_to_last_trajectory_point_,trajectory_dimension_) = trajectory_velocities_.block(0,0,index_to_last_trajectory_point_,trajectory_dimension_);
                new_trajectory_velocities.block(index_to_last_trajectory_point_,0,trajectory.index_to_last_trajectory_point_,trajectory_dimension_) = trajectory.trajectory_velocities_.block(0,0,trajectory.index_to_last_trajectory_point_,trajectory_dimension_);
                trajectory_velocities_ = new_trajectory_velocities;

                MatrixXd new_trajectory_accelerations = MatrixXd::Zero(trajectory_length_, trajectory_dimension_);
                new_trajectory_accelerations.block(0,0,index_to_last_trajectory_point_,trajectory_dimension_) = trajectory_accelerations_.block(0,0,index_to_last_trajectory_point_,trajectory_dimension_);
                new_trajectory_accelerations.block(index_to_last_trajectory_point_,0,trajectory.index_to_last_trajectory_point_,trajectory_dimension_) = trajectory.trajectory_accelerations_.block(0,0,trajectory.index_to_last_trajectory_point_,trajectory_dimension_);
                trajectory_accelerations_ = new_trajectory_accelerations;
            }
        }
        ...
        {
            trajectory_positions_.block(index_to_last_trajectory_point_,0,trajectory.index_to_last_trajectory_point_,trajectory_dimension_) = trajectory.trajectory_positions_.block(0,0,trajectory.index_to_last_trajectory_point_,trajectory_dimension_);
            
            if(!positions_only_)
            {
                trajectory_velocities_.block(index_to_last_trajectory_point_,0,trajectory.index_to_last_trajectory_point_,trajectory_dimension_) = trajectory.trajectory_velocities_.block(0,0,trajectory.index_to_last_trajectory_point_,trajectory_dimension_);
                trajectory_accelerations_.block(index_to_last_trajectory_point_,0,trajectory.index_to_last_trajectory_point_,trajectory_dimension_) = trajectory.trajectory_accelerations_.block(0,0,trajectory.index_to_last_trajectory_point_,trajectory_dimension_);
            }
        }
                                              ...
                                              Eigen::VectorXd &next)
    {
        ...
        {
            Logger::logPrintf(start.size() != POS_VEL_ACC, "Start vector has wrong size >%i<, should be >%i<. Cannot compute next minimum jerk step.", Logger::ERROR, start.size(), POS_VEL_ACC);
            Logger::logPrintf(goal.size() != POS_VEL_ACC, "Goal vector has wrong size >%i<, should be >%i<. Cannot compute next minimum jerk step.", Logger::ERROR, goal.size(), POS_VEL_ACC);
            Logger::logPrintf(next.size() != POS_VEL_ACC, "Next vector has wrong size >%i<, should be >%i<. Cannot compute next minimum jerk step.", Logger::ERROR, next.size(), POS_VEL_ACC);
            return false;
        }
