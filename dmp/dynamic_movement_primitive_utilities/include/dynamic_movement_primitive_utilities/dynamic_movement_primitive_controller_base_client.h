/*************************************************************************
	> File Name: dynamic_movement_primitive_controller_base_client.h
	> Author: Yi Zheng 
	> Mail: hczhengcq@gmail.com
	> Created Time: Wed 07 Dec 2016 10:04:27 AM PST
 ************************************************************************/

#ifndef _DYNAMIC_MOVEMENT_PRIMITIVE_CONTROLLER_BASE_CLIENT_H
#define _DYNAMIC_MOVEMENT_PRIMITIVE_CONTROLLER_BASE_CLIENT_H
...
namespace dmp_utilities
{
    ...
    class DynamicMovementPrimitiveControllerBaseClient
    {
        public:
            ...
            virtual ~DynamicMovementPrimitiveControllerBaseClient() {};
             ...
             */
            void setSingleThreadedMode(bool single_threaded_mode_ = true);
             ...
             */
            bool sendCommand(MessageType& dmp_msg, bool wait_for_success = true);
            ...
            /**
             * Wait for the completion of the last sent command 
    ...
    bool DynamicMovementPrimitiveControllerBaseClient<DMPType, MessageType>::initialize(const std::string& topic_name)
    {
        ...
        status_subscriber_ = node_handle_.subscribe(status_topic_name, 10, &DynamicMovementPrimitiveControllerBaseClient<DMPType, MessageType>::statusCallback, this);
    ...
    {
        ROS_DEBUG("Got >%s< controller status callbacks.", DMPType::getVersionString().c_str());

        if (msg->id != cur_dmp_id_)
        {
            ROS_WARN("Received callback with ID >%s< which is not equal to current DMP id >%i<. Ignoring it.", msg->id, cur_dmp_id_);
            return;
        }
    ...
    template<class DMPType, class MessageType>
    bool DynamicMovementPrimitiveControllerBaseClient<DMPType, MessageType>::sendCommand(MessageType& dmp_msg, bool wait_for_success)
    {
        {
            boost::mutex::scoped_lock lock(dmp_status_mutex_);
            ...
            cur_dmp_id_++;
            ROS_INFO("Sending DMP to a controller. Current DMP is is >%i<.", cur_dmp_id_);

            // set id and set DMP 
            dmp_msg.dmp.state.id = cur_dmp_id_;

            ROS_DEBUG("Sending out DMP of type >%s< - Numebr of Subscribers is >%i<.", DMPType::getVersionString().c_str(), command_publisher_.getNumSubscribers());
    ...
    bool DynamicMovementPrimitiveControllerBaseClient<DMPType, MessageType>::waitForCompletion()
    {
        ...
        if ((last_dmp_status_.status == dynamic_movement_primitive::ControllerStatusMsg::FINISHED) || (last_dmp_status_.status == dynamic_movement_primitive::ControllerStatusMsg::PREEMPTED))
        {
            return true;
        }
